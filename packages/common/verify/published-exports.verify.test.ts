...
describe('published exports after build', () => {
  it('publishConfig.exports targets exist in lib and declarations are non-empty', () => {
  ...
  it('built entry exposes the type-guard API', async () => {
    const mod = await import(
      join(repoRoot, 'packages', PACKAGE, 'lib', `${PACKAGE}.js`)
    );
    expect(typeof mod.isNodeRef).toBe('function');
    expect(typeof mod.toNodeRef).toBe('function');
    expect(typeof mod.isEdgeModel).toBe('function');
  });
});
